...
markup_casino.row('Pull the lever! 💰', 'Change the bet')
markup_casino.row('Exit the casino', 'Game rules 📝')
markup_default = types.ReplyKeyboardMarkup(row_width=2)
markup_default.row('Go to the casino', 'Go to ...')
markup_default.row('Inventory', 'more?')
...
def repeat_all_messages(message):
    if message.text == "Go to the casino":
    ...
    elif message.text == "Pull the lever! 💰":
        if slot_machine.cash >= slot_machine.credit:
            ...
            bot.send_message(message.chat.id, "Your bet " + str(slot_machine.cash) + " $!\n" +
        ...
        bot.send_message(message.chat.id, "Приветствую тебя, игрок! Игра очень проста, каждый игровой ход проходит"
                ...
                "3) Победными являются ряды из 3 одниковых элементов расположенных:\nпо горизонтали\n 🍒🍒 🍒\n"
                        +"по вертикали\n🍏\n🍏\n🍏\nпо диагонали\n7️⃣\n    7️⃣\n         7️⃣\n"
                         ...
                         reply_markup=markup_casino)
    elif message.text == "Exit the casino":
        ...
        if int(message.text) < slot_machine.credit:
            ...
            bot.send_message(message.chat.id, "New bet " + message.text + "$ принята!", reply_markup=markup_casino)
        else:
            bot.send_message(message.chat.id, "Bet not accepted. Insufficient funds: " +
